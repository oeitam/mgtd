...
class Gtd(object):
    ...
    def take_data(self,data):
        ...
        if r"start @" in data:
            ...
            l2 += list(gdb.dft[gdb.dft.State == 'OnHold'].index)
            l2 += list(gdb.dfp[gdb.dfp.State == 'OnHold'].index)
            l4 =  list(gdb.dfa[gdb.dfa.State == 'Dormant'].index)
            l4 += list(gdb.dft[gdb.dft.State == 'Dormant'].index)
            l3 = l1 + l2 + l4
            if len(l3) == 0:
                raise ValueError('for some reason, got an empty list in @0000 replacement')
            r = randint(0,len(l3)-1)
    ...
    def sanitize_input(self):
        ...
        if '|' in self.current_data:
            ...
            gdb.set_trans_description(t3.lstrip())

        ########################################################
        # pipe substitutions => 'simple substitution' but only of what is before the line.
        #   this way - can easily have like same 'task @someproject' and then all different at entering what
        #   is after teh pipe |
...
def tokenize_weekly(command):
    ...
    #for t in tokenize.tokenize(BytesIO("x+1".encode('utf-8')).readline):
        ...
        # when code was written, 59 was the encoding, but now it is 62. Note sure why
        # but note that 59 code is: ERRORTOKEN = 59
        if t[0] == tokenize.ENCODING : # (62) this is the 'first' thing tokenize returns, which is the encoding
            continue
        if t[0] == tokenize.NEWLINE : #(4) trying to work around an issue where at the end of the command it returns a newline (4) instead of endmarker
            continue
        try:
            ...
            if t[0] == tokenize.ENDMARKER:
                ...
                raise SyntaxError("Syntax error t[0] {} t[1] {}".format(t[0], t[1]))
    yield "(end)", "(end)"
...
def nud(self):
    ...
    #     advance()
        ...
        if token.value is not None:
            ...
            if gdb.tag == 'any-tag-at-all':
                raise ValueError("Illegal tag value")
    elif gdb.transaction_type == "swap":
        if gdb.use_this_ID_for_ref == -1:
            gdb.use_this_ID_for_ref = int(token.value)  # get the id relate to to the action
            advance()
        else:
            gdb.second_ref_ID = int(token.value)  # get the id relate to to the action
    elif gdb.transaction_type == 'add comment':
        gdb.use_this_ID_for_ref = int(token.value)  # get the id relate to to the action
        advance()
    elif gdb.transaction_type == 'set priority':
        gdb.use_this_ID_for_ref = int(token.value)  # get the id relate to to the action 
        advance()
        if token.id != '(end)':
...
def nud(self):
    ...
    if token.id == '@':
        ...
        self.second = expression()
        return self
    elif token.value == 'html':
        gdb.transaction_is('list html')
        self.second = expression()
        return self
    elif token.id == 'tag':
...
def nud(self):
    logger.debug("comment nud")
    # creating a project
    gdb.transaction_type = 'add comment' # this is the command type
